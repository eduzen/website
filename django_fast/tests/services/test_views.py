...
class CacheExplorerViewTests(TestCase):
    ...
    def test_access_for_staff(self) -> None:
        self.client.login(username="staff", password="password")
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Cache Explorer")

    def test_access_for_non_staff(self) -> None:
    ...
    def test_render_cache_settings(self) -> None:
        self.client.login(username="staff", password="password")
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_status_dict_contains_expected_status(self) -> None:
        self.client.login(username="staff", password="password")
        response = self.client.get(self.url)
